...
class BackupListView(APIView):
    """GET /api/backups/ — metadata only (never the encrypted bytes; see
    BackupDownloadView for that, gated by the stricter backup.download).
    """
...
class BackupRunView(APIView):
    """POST /api/backups/run/ — triggers backup_db synchronously. Manual
    trigger for the Backups admin page; cron calls the management command
    directly (see deploy/crontab.example) and doesn't go through this view.
    """
...
class BackupDownloadView(APIView):
    """GET /api/backups/<id>/download/ — serves the still-encrypted file
    as-is. Decryption is a deliberately offline, manual step (openssl/the
    restore_db command) so a compromised API session never yields a
    plaintext dump.
    """
