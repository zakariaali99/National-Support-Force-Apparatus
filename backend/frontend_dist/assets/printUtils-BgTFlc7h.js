import{a as e,n as t}from"./api-DVjQ8yrN.js";var n=e(`printer`,[[`path`,{d:`M6 18H4a2 2 0 0 1-2-2v-5a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v5a2 2 0 0 1-2 2h-2`,key:`143wyd`}],[`path`,{d:`M6 9V3a1 1 0 0 1 1-1h10a1 1 0 0 1 1 1v6`,key:`1itne7`}],[`rect`,{x:`6`,y:`14`,width:`12`,height:`8`,rx:`1`,key:`1ue0tg`}]]);function r(e,n=!0){let r=t.getAccess(),i=e,a=i.includes(`?`)?`&`:`?`;if(n&&!i.includes(`html=`)&&(i=`${i}${a}html=1`),r&&!i.includes(`token=`)){let e=i.includes(`?`)?`&`:`?`;i=`${i}${e}token=${encodeURIComponent(r)}`}return`/api/${i.replace(/^\/?api\//,``)}`}function i(e){let t=r(e,!0);window.open(t,`_blank`)}function a({title:e,subtitle:t,department:n=`إدارة الشؤون الإدارية والعهد`,docType:r=`كشف رسمي معتمد`,documentNumber:i,orientation:a=`portrait`,contentHtml:o,showSignatures:s=!0}){let c=a===`landscape`,l=c?1200:920,u=c?850:1080,d=window.open(``,`_blank`,`width=${l},height=${u},menubar=no,toolbar=no,location=no,status=no`);if(!d){alert(`يرجى السماح بالنوافذ المنبثقة (Popups) لعرض وطباعة المستند في نافذة مستقلة.`);return}let f=new Date().toLocaleDateString(`ar-LY`,{year:`numeric`,month:`long`,day:`numeric`}),p=new Date().toLocaleTimeString(`ar-LY`,{hour:`2-digit`,minute:`2-digit`}),m=i||`NASF-${Math.floor(1e5+Math.random()*9e5)}`,h=`<!DOCTYPE html>
...
<body>
  ...
  <div class="document-page">
...
</html>`;d.document.open(),d.document.write(h),d.document.close()}function o({item:e={},domain:t=`inventory`,type:n,history:r=[]}){let i=t===`armory`||n===`weapon`||!!(e.serial_number||e.caliber),o=i?`بطاقة صنف سلاح وتجهيز تسليحي: ${e.name||`صنف تسليحي`}`:`بطاقة صنف ومواصفات مستودعية: ${e.name||`صنف مستودعي`}`,s=i?`إدارة التسليح والذخائر`:`المستودع والمخازن العامة`,c=i?`بطاقة صنف تسليحي`:`بطاقة صنف مخزني`,l=Array.isArray(r)&&r.length>0?`
    ...
    <div class="form-grid">
      ...
      <div class="form-row"><span class="form-label">${i?`العيار / المواصفة:`:`كود الصنف:`}</span><span class="form-value">${e.caliber||e.item_code||`—`}</span></div>
    ...
    ${l}
  `;a({title:o,subtitle:`كود الصنف: ${e.serial_number||e.item_code||`NASF-ITEM`} | الحالة العامة: ممتاز وجاهز للصرف`,department:s,docType:c,documentNumber:`CARD-${(e.id||Date.now().toString().slice(-5)).toString().padStart(5,`0`)}`,orientation:`portrait`,contentHtml:u})}function s({custody:e,custodyRecord:t,item:n={},member:r={},action:i=`صرف عهدة`,voucherNumber:o}){let s=t||e||{},c=!!(n.serial_number||n.caliber||s.serial_number),l=`محضر ${i} رسمي معتمد`,u=c?`إدارة التسليح والذخائر`:`إدارة المستودعات والعهد العامة`,d=r?.full_name||s.member_name||s.recipient_name||`—`,f=r?.force_number||s.force_number||s.recipient_force_number||`—`,p=r?.rank_name||s.rank_name||s.recipient_rank||`عضو`,m=r?.faction_name||s.faction_name||s.recipient_faction||`عام`,h=r?.national_number||s.national_number||`—`,g=r?.phone_number||s.phone||s.phone_number||`—`,_=n?.name||s.item_name||`—`,v=s.quantity||e?.quantity||1,y=c?n.serial_number||s.serial_number||n.caliber||`—`:n.item_code||s.item_code||`—`,b=`
    <div class="section-title">بيانات المستلم / صاحب العهدة</div>
    <div class="form-grid">
      <div class="form-row"><span class="form-label">اسم المستلم:</span><span class="form-value">${d}</span></div>
      <div class="form-row"><span class="form-label">الرقم العسكري:</span><span class="form-value font-mono">${f}</span></div>
      <div class="form-row"><span class="form-label">الرتبة العسكرية:</span><span class="form-value">${p}</span></div>
      <div class="form-row"><span class="form-label">الإدارة / الفصيل:</span><span class="form-value">${m}</span></div>
      <div class="form-row"><span class="form-label">الرقم الوطني:</span><span class="form-value font-mono">${h}</span></div>
      <div class="form-row"><span class="form-label">رقم الهاتف:</span><span class="form-value font-mono">${g}</span></div>
    </div>

    <div class="section-title">بيانات الصنف أو السلاح المسلّم</div>
    <div class="form-grid">
      <div class="form-row"><span class="form-label">الصنف المسلّم:</span><span class="form-value">${_}</span></div>
      <div class="form-row"><span class="form-label">الكمية المسلّمة:</span><span class="form-value font-mono" style="font-weight: 800;">${v} قطعة</span></div>
      <div class="form-row"><span class="form-label">${c?`الرقم التسلسلي (Serial):`:`كود القطعة:`}</span><span class="form-value font-mono">${y}</span></div>
  ...
  `;a({title:l,subtitle:`رقم إذن الصرف: ${o||s.id||`VCH-001`} | تاريخ التسليم: ${new Date().toLocaleDateString(`ar-LY`)}`,department:u,docType:`محضر تسليم واستلام عهدة`,documentNumber:o||`VOUCHER-${s.id||Date.now().toString().slice(-5)}`,orientation:`portrait`,contentHtml:b})}function c({vehicle:e={},trip:t={},tripNumber:n}){let r=e.name||`الآلية والمركبة`,i=n||t.id||`TRIP-${Date.now().toString().slice(-5)}`,o=`
    <div class="section-title">بيانات الآلية والمركبة</div>
    <div class="form-grid">
      <div class="form-row"><span class="form-label">المركبة والطراز:</span><span class="form-value">${r}</span></div>
    ...
    <div class="form-grid">
      ...
      <div class="form-row"><span class="form-label">وجهة التحرك:</span><span class="form-value">${t.destination||`وفق خط السير المعتمد`}</span></div>
      ...
      <div class="form-row"><span class="form-label">الغرض من التحرك:</span><span class="form-value">${t.purpose||`مهمة إدارية / عملياتية رسمية`}</span></div>
    </div>
  ...
  `;a({title:`أمر تحرك ومهمة مركبة آلية رسمي`,subtitle:`رقم أمر التحرك: ${i} | المركبة: ${r}`,department:`إدارة النقليات والآليات`,docType:`أمر تحرك رسمي`,documentNumber:i,orientation:`portrait`,contentHtml:o})}var l=c;function u({items:e=[],domain:t=`inventory`,filtersSummary:n=``}){let r=t===`armory`,i=r?`كشف حصر وجرد مستودع التسليح والأسلحة والذخائر الرسمي`:`كشف حصر وجرد المستودع والمخازن العامة الرسمي`,o=r?`إدارة التسليح والذخائر`:`المستودع والمخازن العامة`,s=r?`كشف حصر تسليحي`:`كشف جرد مستودع`,c=n?`<div class="filter-summary-box"><strong>معايير التصفية المطبقة:</strong> ${n}</div>`:``,l=e.length===0?`<tr><td colspan="8" style="text-align: center; padding: 18px; color: #64748b;">لا توجد أصناف مسجلة في هذا الكشف.</td></tr>`:e.map((e,t)=>`
        <tr>
          <td style="text-align: center; font-weight: 700;">${t+1}</td>
  ...
  `;a({title:i,subtitle:`تاريخ الجرد: ${new Date().toLocaleDateString(`ar-LY`)} | إجمالي الأصناف المشمولة: ${e.length}`,department:o,docType:s,documentNumber:`INV-SUM-${Date.now().toString().slice(-6)}`,orientation:r?`portrait`:`landscape`,contentHtml:c+u})}function d({vehicles:e=[],filtersSummary:t=``}){let n=t?`<div class="filter-summary-box"><strong>معايير التصفية المطبقة:</strong> ${t}</div>`:``,r=`
  ...
  `;a({title:`كشف حصر وجرد أسطول الآليات والمركبات الرسمي`,subtitle:`تاريخ الحصر: ${new Date().toLocaleDateString(`ar-LY`)} | إجمالي المركبات: ${e.length}`,department:`إدارة النقليات والآليات`,docType:`كشف حصر أسطول الآليات`,documentNumber:`VEH-SUM-${Date.now().toString().slice(-6)}`,orientation:`landscape`,contentHtml:n+r})}function f({logs:e=[],filtersSummary:t=``}){let n=t?`<div class="filter-summary-box"><strong>معايير التصفية المطبقة:</strong> ${t}</div>`:``,r=`
  ...
  `;a({title:`كشف حصر وبيانات أفراد القوة الرسمية`,subtitle:`كشف حصر أفراد القوة والمنتسبين | إجمالي الأفراد: ${e.length}`,department:`شعبة شؤون الأفراد والضباط`,docType:`كشف حصر قوة عسكرية`,documentNumber:`MEM-SUM-${Date.now().toString().slice(-6)}`,orientation:`landscape`,contentHtml:n+r})}function m({member:e={}}){let t=`استمارة وبيانات قيد فرد: ${e.full_name||`عضو`}`,n=e.faction_name?`إدارة / ${e.faction_name}`:`شعبة شؤون الأفراد والضباط`,r=e.force_number||`—`,i=e.photo_url||e.photo||e.photo_thumb_url,o=`
    <div style="display: flex; align-items: center; justify-content: space-between; gap: 20px; background: #f8fafc; border: 1.5px solid #cbd5e1; border-radius: 10px; padding: 14px 18px; margin-bottom: 18px;">
      <div style="width: 95px; height: 120px; border: 1.5px solid #0a2540; border-radius: 6px; background: #ffffff; overflow: hidden; display: flex; align-items: center; justify-content: center; flex-shrink: 0;">
        ${i?`<img src="${i}" alt="صورة الفرد" style="width: 100%; height: 100%; object-fit: cover;" onerror="this.parentElement.innerHTML='<span style=\\'font-size: 10px; color: #94a3b8; font-weight: bold;\\'>صورة شخصية</span>'" />`:`<div style="display: flex; align-items: center; justify-content: center; height: 100%; font-size: 11px; font-weight: 700; color: #94a3b8; text-align: center; line-height: 1.4;">صورة الفرد<br>(غير متوفرة)</div>`}
      </div>
      <div style="flex: 1;">
  ...
  `;a({title:t,subtitle:`الرقم الحربي: ${r} | الرتبة: ${e.rank_name||`—`} | الإدارة: ${e.faction_name||`—`}`,department:n,docType:`استمارة قيد وبيانات فرد`,documentNumber:r,orientation:`portrait`,contentHtml:o+s+c})}export{u as a,l as c,s as i,d as l,f as n,m as o,i as r,p as s,o as t,n as u};