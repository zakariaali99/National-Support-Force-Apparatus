import{t as e}from"./api-DVjQ8yrN.js";import{H as t,it as n}from"./index-xLrNMrlM.js";function r(t){let r=window.open(``,`_blank`);r&&r.document.write(`
      <!DOCTYPE html>
    ...
    `);let i=t.replace(/^\/?api\//,``),a=i.includes(`?`)?`&`:`?`,o=i.includes(`html=`)?i:`${i}${a}html=1`;e.get(o,{responseType:`text`}).then(({data:e})=>{if(r&&!r.closed)r.document.open(),r.document.write(e),r.document.close();else{let t=URL.createObjectURL(new Blob([e],{type:`text/html;charset=utf-8`}));window.open(t,`_blank`)}}).catch(e=>{r&&!r.closed&&r.close(),console.error(`Print error:`,e),n(`تعذر فتح كشف الطباعة — حاول مرة أخرى`,`error`)})}function i(){try{let e=localStorage.getItem(`nsfa_current_user`);if(e)return JSON.parse(e)}catch{}return null}function a({title:e,subtitle:n,department:r=`إدارة الشؤون الإدارية والعهد`,docType:a=`كشف رسمي معتمد`,documentNumber:o,orientation:s=`portrait`,contentHtml:c,showSignatures:l=!0}){let u=s===`landscape`,d=u?1200:920,f=u?850:1080,p=window.open(``,`_blank`,`width=${d},height=${f},menubar=no,toolbar=no,location=no,status=no`);if(!p){alert(`يرجى السماح بالنوافذ المنبثقة (Popups) لعرض وطباعة المستند في نافذة مستقلة.`);return}let m=new Date().toLocaleDateString(`ar-LY`,{year:`numeric`,month:`long`,day:`numeric`}),h=new Date().toLocaleTimeString(`ar-LY`,{hour:`2-digit`,minute:`2-digit`}),g=o||`NASF-${Math.floor(1e5+Math.random()*9e5)}`,_=i(),v=_?.full_name||(_?.first_name?`${_.first_name} ${_.last_name}`.trim():_?.username)||`.......................................`,y=_?.roles?.map(e=>e.name_ar).join(` / `)||(_?.is_superuser?`مسؤول المنظومة`:`مسؤول السجل والتوثيق`),b=_?.faction_name||``,x=b?`${y} — ${b}`:y,S=`<!DOCTYPE html>
...
<head>
  ...
  <style>
    *, *::before, *::after {
    ...
    .document-page {
      width: ${u?`297mm`:`210mm`};
      min-height: ${u?`188mm`:`272mm`};
      margin: 20px auto;
      background: #ffffff;
      padding: ${u?`10mm 12mm`:`12mm 14mm`};
    ...
    @media print {
      ...
      @page {
        size: ${u?`A4 landscape`:`A4 portrait`};
        margin: ${u?`8mm 10mm`:`10mm 12mm`};
      }
    }
  </style>
</head>
<body>
  ...
  <div class="document-page">
    ...
    <div class="gov-header-container">
      ...
      <div class="gov-header-metadata">
        ...
        <span>الرقم المرجعي للمستند: ${g}</span>
      </div>
    </div>
    ...
    <div class="bottom-endorsement-block">
      ${l?`
        <div class="signatures-block">
          <div class="sig-box">
            <p class="sig-title">مسؤول السجل والتوثيق (${x})</p>
            <p class="sig-sub" style="font-weight: 700; color: #0a2540; margin-top: 4px;">الاسم: ${v}</p>
      ...
      <div class="gov-print-footer">
        <span>تاريخ الطباعة: ${m}</span>
        <span>توقيت الإصدار: ${h}</span>
      </div>
    </div>
  </div>
</body>
</html>`;p.document.open(),p.document.write(S),p.document.close()}function o({item:e={},domain:t=`inventory`,type:n,history:r=[]}){let i=t===`armory`||n===`weapon`||!!(e.serial_number||e.caliber),o=i?`بطاقة صنف سلاح وتجهيز تسليحي: ${e.name||`صنف تسليحي`}`:`بطاقة صنف ومواصفات مستودعية: ${e.name||`صنف مستودعي`}`,s=i?`إدارة التسليح والذخائر`:`المستودع والمخازن العامة`,c=i?`بطاقة صنف تسليحي`:`بطاقة صنف مخزني`,l=Array.isArray(r)&&r.length>0?`
    ...
    ${l}
  `;a({title:o,subtitle:`كود الصنف: ${e.serial_number||e.item_code||`NASF-ITEM`} | الحالة العامة: ممتاز وجاهز للصرف`,department:s,docType:c,documentNumber:`CARD-${(e.id||Date.now().toString().slice(-5)).toString().padStart(5,`0`)}`,orientation:`portrait`,contentHtml:u})}function s({custody:e,custodyRecord:t,item:n={},member:r={},action:i=`صرف عهدة`,notes:o,voucherNumber:s}){let c=t||e||{},l=!!(n.serial_number||n.caliber||c.serial_number),u=`محضر ${i} رسمي معتمد`,d=l?`إدارة التسليح والذخائر`:`إدارة المستودعات والعهد العامة`,f=r?.full_name||c.member_name||c.recipient_name||`—`,p=r?.force_number||c.force_number||c.recipient_force_number||`—`,m=r?.rank_name||c.rank_name||c.recipient_rank||`عضو`,h=r?.faction_name||c.faction_name||c.recipient_faction||`عام`,g=r?.national_number||c.national_number||`—`,_=r?.phone_number||c.phone||c.phone_number||`—`,v=n?.name||c.item_name||`—`,y=c.quantity||e?.quantity||1,b=l?n.serial_number||c.serial_number||n.caliber||`—`:n.item_code||c.item_code||`—`,x=o||c.notes||e?.notes||n?.notes||``,S=`
  ...
  `;a({title:`أمر تحرك ومهمة مركبة آلية رسمي`,subtitle:`رقم أمر التحرك: ${o} | المركبة: ${i}`,department:`إدارة النقليات والآليات`,docType:`أمر تحرك رسمي`,documentNumber:o,orientation:`portrait`,contentHtml:c})}var l=c;function u({items:e=[],domain:t=`inventory`,filtersSummary:n=``}){let r=t===`armory`,i=r?`كشف حصر وجرد مستودع التسليح والأسلحة والذخائر الرسمي`:`كشف حصر وجرد المستودع والمخازن العامة الرسمي`,o=r?`إدارة التسليح والذخائر`:`المستودع والمخازن العامة`,s=r?`كشف حصر تسليحي`:`كشف جرد مستودع`,c=n?`<div class="filter-summary-box"><strong>معايير التصفية المطبقة:</strong> ${n}</div>`:``,l=e.length===0?`<tr><td colspan="8" style="text-align: center; padding: 18px; color: #64748b;">لا توجد أصناف مسجلة في هذا الكشف.</td></tr>`:e.map((e,t)=>`
        <tr>
          <td style="text-align: center; font-weight: 700;">${t+1}</td>
  ...
  `;a({title:`كشف حصر وبيانات أفراد القوة الرسمية`,subtitle:`كشف حصر أفراد القوة والمنتسبين | إجمالي الأفراد: ${e.length}`,department:`شعبة شؤون الأفراد والضباط`,docType:`كشف حصر قوة عسكرية`,documentNumber:`MEM-SUM-${Date.now().toString().slice(-6)}`,orientation:`landscape`,contentHtml:n+r})}export{u as a,d as c,s as i,f as n,p as o,r,l as s,o as t};